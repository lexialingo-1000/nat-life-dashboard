...
// dashboard-23 R2 — masquer aussi les tâches VALIDÉ par défaut (comme terminé).
const DEFAULT_VISIBLE_STATUS = ALL_STATUS_VALUES.filter((v) => v !== 'termine' && v !== 'valide');
    ...
    <tr className="hover:bg-[#fbf8f0]/40">
      ...
      <td className="px-3 py-2 text-right">
        <div className="inline-flex items-center gap-1">
          ...
          {/* dashboard-23 R1 — suppression de tâche depuis la liste plate (parité avec l'arbre). */}
          <DeleteTacheButton id={tache.id} title={tache.title} returnTo={returnTo} />
        </div>
      </td>
    </tr>
  );
...
// dashboard-23 R1 — bouton de suppression inline d'une tâche (copié de
// marches-tree.tsx). useTransition + confirm() ; deleteTacheAction revalide /biens
// et redirige sur returnTo pour rafraîchir la liste.
    ...
    <MobileCard
      ...
      actions={
        <>
          ...
          {/* dashboard-23 R1 — suppression de tâche depuis la carte mobile. */}
          <DeleteTacheButton id={tache.id} title={tache.title} returnTo={returnTo} />
        </>
      }
    />
