...
const s = StyleSheet.create({
  ...
  sousLotHeader: {
    // dashboard-24 — sous-lot souligné + espace au-dessus (lisibilité, demande client).
    borderLeftWidth: 2,
  ...
  photosRow: {
    ...
    gap: 4,
    marginTop: 4,
    // dashboard-23 R6c — photo alignée à gauche (flush, plus d'indentation).
    marginLeft: 0,
  },
  photo: {
    // dashboard-24 — @react-pdf n'honore pas objectFit:'cover' (garde le ratio,
    // déborde la box) → une photo paysage sortait à 96px de large et se décalait.
    // Largeur fixe seule = toutes les photos à 72px de large, calées à gauche,
    // hauteur auto selon le ratio (pas de distorsion).
    width: 72,
    borderRadius: 2,
  },
  footer: {
              ...
              {marche.sousLots.map((sousLot) => (
                <View key={sousLot.id} style={s.sousLot}>
                  ...
                  {sousLot.taches.map((tache) => {
                    // dashboard-23 R6b — ordre imposé : Statut / Désignation / Niveau / Pièce / Date échéance.
                    return (
                        ...
                        <View style={s.tache}>
                          ...
                          <View style={s.tacheBody}>
                            ...
                            >
                              ...
                              {/* 3 — Niveau */}
                              {tache.levelName && (
                                <Text style={s.tacheMeta}>Niveau : {tache.levelName}</Text>
                              )}
                              {/* 4 — Pièce */}
                              {tache.roomName && (
                                <Text style={s.tacheMeta}>Pièce : {tache.roomName}</Text>
                              )}
                              {/* 5 — Date échéance */}
                              {tache.dueDate && (
