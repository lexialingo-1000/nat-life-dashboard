...
import {
  ...
  type CreateDocumentCategoryState,
} from './actions';

const initialState: CreateDocumentCategoryState = { status: 'idle' };
  ...
  return (
    <button type="submit" disabled={pending} className="btn-primary">
      {pending ? (
        ...
        <><Plus className="mr-1.5 h-3.5 w-3.5" strokeWidth={2} />Créer la catégorie</>
      )}
    </button>
  );
}
...
export function DocumentCategoryCreateForm() {
  const [state, formAction] = useFormState(createDocumentCategoryAction, initialState);
    ...
    <form ref={formRef} action={formAction} className="card space-y-4 p-6">
      ...
      {state.status === 'success' && (
        <div className="flex items-start gap-2 rounded-md border border-blue-200 bg-blue-50 p-3">
          ...
          <div className="text-[13px] text-blue-800">
            Catégorie créée avec succès. Visible dans la liste ci-dessus.
          </div>
        </div>
      )}
      ...
      <div className="grid grid-cols-2 gap-4">
        <div>
          ...
          <input name="code" required className="input mt-1 font-mono" placeholder="urbanisme" />
        ...
        <div>
          ...
          <input name="label" required className="input mt-1" placeholder="Urbanisme" />
        </div>
      </div>
