...
export default async function EditDocumentCategoryPage({ params }: { params: { id: string } }) {
  const rows = await db
    ...
    .where(eq(documentCategories.id, params.id))
    .limit(1);
  ...
  return (
    <div className="space-y-8">
      <BackLink fallbackHref="/admin/document-categories" label="Catégories de documents" />
      ...
      <form action={updateDocumentCategoryAction} className="card space-y-5 p-6">
        ...
        <div className="flex items-center justify-between gap-3 pt-2">
          <DeleteButton
            action={deleteDocumentCategoryAction}
            id={t.id}
