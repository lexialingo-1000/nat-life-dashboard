...
import { createSupplierTypeAction, type CreateSupplierTypeState } from './actions';

const initialState: CreateSupplierTypeState = { status: 'idle' };
...
export function SupplierTypeCreateForm() {
  const [state, formAction] = useFormState(createSupplierTypeAction, initialState);
    ...
    <form ref={formRef} action={formAction} className="card space-y-4 p-6">
      ...
      <div className="grid grid-cols-2 gap-4">
        ...
        <div>
          ...
          <input name="label" required className="input mt-1" placeholder="Huissier" />
        </div>
      </div>
