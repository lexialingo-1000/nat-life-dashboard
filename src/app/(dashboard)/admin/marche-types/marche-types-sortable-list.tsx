...
import { reorderMarcheTypesAction, toggleMarcheTypeActiveAction } from './actions';

export interface MarcheTypeRow {
  id: string;
  label: string;
  code: string;
  isActive: boolean;
...
export function MarcheTypesSortableList({ rows }: Props) {
  const [items, setItems] = useState<MarcheTypeRow[]>(rows);
  ...
  const handleDrop = (targetIndex: number) => {
    ...
    startTransition(async () => {
      await reorderMarcheTypesAction(next.map((r) => r.id));
    });
  };
        ...
        >
          ...
          <td className="pr-5 text-right">
            <div className="inline-flex items-center gap-3">
              <Link
                href={`/admin/marche-types/${t.id}/edit`}
                className="inline-flex items-center gap-1 text-[12px] text-blue-700 hover:text-blue-800"
              >
                <Pencil className="h-3 w-3" strokeWidth={2} />
                Modifier
              </Link>
              <form action={toggleMarcheTypeActiveAction} className="inline-block">
