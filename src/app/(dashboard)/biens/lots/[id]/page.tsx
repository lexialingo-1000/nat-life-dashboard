    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-zinc-500">
            ...
            <span className="rounded-full bg-zinc-100 px-2 py-0.5 text-xs">{lot.status}</span>
          </p>
        </div>
        <Link
          href={`/biens/lots/${lot.id}/edit`}
          className="inline-flex items-center gap-2 rounded-md border border-zinc-200 bg-[#fbf8f0] px-3 py-2 text-sm font-medium text-zinc-700 hover:bg-zinc-50"
        >
      ...
      <div className="card p-6">
        ...
        <dl className="mt-4 grid grid-cols-2 gap-4 text-sm">
          <div>
            <dt className="text-xs uppercase text-zinc-500">Surface Carrez</dt>
      ...
      <div className="card p-6">
        ...
        {lotLevels.length === 0 ? (
          ...
          <ul className="mt-3 space-y-2">
            {lotLevels.map((lv) => (
              <li key={lv.id} className="rounded-md border border-zinc-100 p-3 text-sm">
                {lv.name}
              </li>
            ))}
          </ul>
